<template>
  <form @submit.prevent="send2API" class="column small-12">
    ...
    <div v-show="$store.state.step === 0 && !thanks">
      ...
      <div class="row">
        <p class="faded column small-12">
          Az email címed a kitöltés ellenőrzéséhez kell
        </p>
      </div>
      <div class="row">
        <input
    ...
    <div class="row align-center" v-show="$store.state.step < 7 && !thanks">
      <button
        ...
        class="button"
        @click.prevent="$store.commit('increaseStep')"
      >
        Start!
      </button>
      <button
        v-show="$store.state.step"
        class="button"
        @click.prevent="$store.commit('decreaseStep')"
      >
    ...
    <div v-show="$store.state.step === 7">
      <div class="row">
        <label for="newsletter" class="column small-12"
          >Hírlevél feliratkozás</label
        >
      </div>
      <div class="row">
        <p class="faded column small-12">
          Maradjunk kapcsolatban! Íratkozz fel a hírlevelünkre. 1-2 emailt
          küldünk havonta sok érdekeséggel!
        </p>
      </div>
      <div class="row">
        <input
        ...
        <p class="faded column small-12">
          Elfogadom a krisnavolgy.hu oldalon található adatkezelési
          tájékoztatót, az adatkezeléshez hozzájárulok.
        </p>
      </div>
            ...
            marketing_permissions: [
              {
                marketing_permission_id:
                  process.env.VUE_APP_MAILCHIMP_MARKETING_PERMISSION_ID,
                text: "Elfogadom",
                enabled: true
              }
            ]
